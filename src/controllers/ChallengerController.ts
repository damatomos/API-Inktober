...
router.post('/', async (req, res) => {
	...
	const result = await Challenger.where({ day, year }).findOne().exec();
...
router.get('/', async (req, res) => {
	...
	try {
		let result = await Challenger.find(year ? { year } : {}).sort('year').sort('day').exec();
		let years = [];
...
router.get('/:id', async (req, res) => {
	...
	try {
		if (id) {
			const result = await Challenger.findById(id);
			if (result){
				res.statusCode = 200;
...
router.delete('/:id', async (req, res) => {
	...
	try {
		if (id){
			const result = await Challenger.findOneAndDelete({_id: id}).exec();
			if (result) {
				res.statusCode = 200;
...
router.put('/:id', async (req, res) => {
	...
	const challenger = await Challenger.findById(id);

	if ( validateItems(name, day, year) && id && challenger) {
		try {
			const result = await Challenger.where({_id: id}).updateOne({name, day, year});
			res.statusCode = 200;
...
router.delete('/deadend/on', async (req, res) => {
	try {
		const result = await Challenger.deleteMany({});
		res.statusCode = 200;
